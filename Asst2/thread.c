...
//These initizlize at thread_bootstrap
static int MaxPid;
static struct thread **pidArray;	//array of pointers to threads
static int pidIndex;
...
//Assign thread pid and save thread in array in Pid array
void assignPid(struct thread* thread){
	if( pidIndex < MaxPid + 1){
		pidArray[pidIndex] = thread;
		thread->pid = pidIndex;
		pidIndex ++ ;
		return;
	}
	//we're at the end of the list and about to run out of pid range
	else//check for free pid to reuse
	{
		int i;
		for(i = 1; i < MaxPid; i++){
			if(pidArray[pidIndex] == NULL){
				pidArray[pidIndex] = thread;
				thread->pid = pidIndex;
				return;
			}
	...
	//so no pid is free to reuse, make a bigger array
	struct thread **temp1 = pidArray;
	struct thread **temp2 = (struct thread *) kmalloc(sizeof(struct thread) * MaxPid * 2);
	if(temp2 == NULL)
	{
		panic("Failed to allocate more memory for pidArray");
	}
	memccpy(temp2, pidArray, MaxPid);
	MaxPid *= 2;
...
thread_bootstrap(void)
{
	...
	//creating array for PID
	MaxPid = 100;
	pidArray = (struct thread *) kmalloc(sizeof(struct thread) * MaxPid);
	if(pidArray == NULL)
	{
